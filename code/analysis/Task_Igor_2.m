...
Tasks=unique(TasksTable(:,1), 'stable'); % Hi Elena, this line has to be changed in order to avoid "unique" function sorts the tasks alphabetically. Check "unique" function documentation
ListOfTasks=unique(join(string(TasksTable{:,2:4}),'_'), 'stable');
ListOfTasksSplit=split(ListOfTasks,'_');
AllTraumaSummaryTable=zeros(height(Tasks),3);
...
set(nucleotide_metabolism,'Linkage','complete','Dendrogram',3, 'ColumnLabels', {'Lung metastasis','Brain metastasis','Breast Cancer'}, 'ColumnLabelsRotate', 45);

carbohydrate_tasks = readtable('GitHub/metastasis/data/pls_da_analysis/carbohydrate_tasks.csv');
carbohydrate_tasks=join(string(carbohydrate_tasks{:,:}),'_');
set(carbohydrate_metabolism, 'RowLabels', cellstr(unique(carbohydrate_tasks(cellfun(@str2num,carbohydrate_metabolism.RowLabels, Uniform)), 'stable')));
set(carbohydrate_metabolism,'Linkage','complete','Dendrogram',3, 'ColumnLabels', {'Lung metastasis','Brain metastasis','Breast Cancer'}, 'ColumnLabelsRotate', 45);
